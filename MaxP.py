...
from entropy import calculate_entropy,probility_entropy_weight1
...
if __name__ == '__main__':
    ...
    for num in range(100, 1100, 100):
        ...
        for i in range(1, 51):  # 进行50次实验
            ...
            acc_val0 = ori_model.evaluate(x_val, np_utils.to_categorical(y_val, 10), verbose=0)[1]
            ...
            # 计算每个测试用例的置信度（1 - 最大概率）
            confidences = 1 - np.max(predicted_probabilities, axis=1)

            # 获取置信度值和对应的索引
            confidence_indices = np.argsort(confidences)[::-1]  # 降序排序获取索引

            # 选择置信度最低的前num个测试用例的索引
            top_indices = confidence_indices[:num]

            # 使用这些索引从原始测试用例中选择数据
            x_top = x_sel[top_indices]
            y_top = y_sel[top_indices]

            # 调用 fault_detection 函数
            fault_num, diverse_fault_num = fault_detection(y_top, y_sel_psedu[top_indices])
            total_mistake += fault_num
            mistake_type += diverse_fault_num

        print("检索前" + str(num) + "时平均结果为")
        print("平均发现错误总数: " + str(total_mistake / 50))
        print("平均发现错误种类: " + str(mistake_type / 50))
